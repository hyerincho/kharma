...
@click.option("--rst_from_oz_file", default=None, help="Restart multizone run from onezone.")
...
@click.option("--half_trun", is_flag=True, help="Half all the runtime.")
@click.option("--r_shell", default=3e6, help="r_shell for GIZMO's case mostly.")
@click.option("--output0_dt", default=None, help="For debugging purposes, can set the dt directly.")
# Don't use this
...
def run_multizone(**kwargs):
    ...
    if kwargs["restart"]:
        ...
        base = kwargs["base"]
        args = {}
        if kwargs["rst_from_oz_file"] is None: args["parthenon/job/problem_id"] = "bondi"
        else:
            args["parthenon/job/problem_id"] = "resize_restart_kharma"
            args["resize_restart/fname"] = kwargs["rst_from_oz_file"]
        ...
        if kwargs["gizmo"]:
            args["bondi/r_shell"] = kwargs['r_shell']  # args['coordinates/r_in'] # not using r_shell
        else:
        ...
        if kwargs["bz"] != 0.0:
            ...
            if kwargs["bclean"]:
                ...
                args["b_cleanup/rel_tolerance"] = 1.e-2
...
def copy_from_restart(kwargs):
    ...
    for arg in kwargs_save.keys():
        if "nlim" in arg or "kharma_bin" in arg or "tmax" in arg or "gamma_max" in arg:  # replace nlim, kharma_bin from previous run with passed arguments
