...
def get_mask(zone, n_zones, n_radii):
    # masking
...
def plot_Mdot_eta(dirtag, ax_passed=None, color='k', lw=1, marker=None, label=None, show_Bondi=False, show_divisions=False, avg=False):
    ...
    if avg:
        ...
        Mdot_in_zones=[None]*n_zones
        Mdot_out_zones=[None]*n_zones
        ...
        for i in range(iteration*(n_zones-1)+1):
            ...
            for file_ in files[len(files)//100:]:#2:]:  # only add last half
                ...
                if Mdot_in_zones[zone] is None: Mdot_in_zones[zone] = calc_Mdot_in(dump) 
                else: Mdot_in_zones[zone] +=  calc_Mdot_in(dump)#1+r_average(dump, dump["ucov"][0]) #
                if Mdot_out_zones[zone] is None: Mdot_out_zones[zone] = calc_Mdot_out(dump) 
                else: Mdot_out_zones[zone] +=  calc_Mdot_out(dump)#1+r_average(dump, dump["ucov"][0]) #
        ...
        for zone in range(n_zones):
            ...
            Mdot_in_zones[zone] /= num_sum[zone]
            Mdot_out_zones[zone] /= num_sum[zone]
            ...
            Mdot_in = np.concatenate([Mdot_in,Mdot_in_zones[zone][mask]])
            Mdot_out = np.concatenate([Mdot_out,Mdot_out_zones[zone][mask]])
    ...
    radii = radii[order]
    Mdot = Mdot[order]
    Mdot_in = Mdot_in[order]
    Mdot_out = Mdot_out[order]
    #if avg:
    #    Edot = Edot[order]
    #    eta = 1.-Edot/Mdot[0]
    #else:
    ...
    ax[0].plot(radii, Mdot_in, color='b', marker=marker, lw=lw, label=r'$\dot{M}_{\rm in}$')
    ax[0].plot(radii, -Mdot_in, color='b', ls=':', marker=marker, lw=lw)
    ax[0].plot(radii, Mdot_out, color='r', marker=marker, lw=lw, label=r'$\dot{M}_{\rm out}$')
    ax[0].plot(radii, -Mdot_out, color='r', ls=':', marker=marker, lw=lw)
...
def FE_adv_slice(dirtag,avg=True, show_Be=True):
    matplotlib_settings()
    ...
    FE_adv_zones=[None]*n_zones
    Be_nob_zones=[None]*n_zones
    ur_mean_zones=[None]*n_zones
    ur2_mean_zones=[None]*n_zones
    ur_var_zones=[None]*n_zones
    num_sum=[0]*n_zones
    r_zones=[None]*n_zones
    dump_zones=[None]*n_zones
    ...
    for i in range(iteration*(n_zones-1)+1):
        ...
        for file_ in files[len(files)//2:]:  # only add last half
            ...
            if ur_mean_zones[zone] is None: ur_mean_zones[zone] = dump["u^r"]
            else: ur_mean_zones[zone] += dump["u^r"]
            if ur2_mean_zones[zone] is None: ur2_mean_zones[zone] = np.power(dump["u^r"],2)
            else: ur2_mean_zones[zone] += np.power(dump["u^r"],2)
            num_sum[zone]+=1
            if r_zones[zone] is None:
                r_zones[zone] = dump["r1d"]
            if dump_zones[zone] is None:
    ...
    for zone in range(n_zones):
        Be_nob_zones[zone] /= num_sum[zone]
        ur_mean_zones[zone] /= num_sum[zone]
        ur2_mean_zones[zone] /= num_sum[zone]
        FM_zones[zone] /= num_sum[zone]
        FE_adv_zones[zone] = Be_nob_zones[zone]*FM_zones[zone] # advection

        mask = get_mask(zone, n_zones, n_radii)
        dump = dump_zones[zone]
        ...
        if show_Be: 
            ...
            plot_xz(ax[0], dump, ur_mean_zones[zone] , native=True, symlog=True, vmin=vmin,vmax=vmax, cbar=0,shading='flat',window=window,average=avg, mask=mask)
            plot_xz(ax[1], dump, ur2_mean_zones[zone] -np.power(ur_mean_zones[zone],2.) , native=True, symlog=True, vmin=vmin,vmax=vmax, cbar=0,shading='flat',window=window,average=False, mask=mask)

    if not show_Be:
        # additionally calculate u^r variance
        for i in range(iteration*(n_zones-1)+1):
            files=sorted(glob.glob(dirs[edge_run-i]+"/*.phdf")) # HYERIN TEST rhdf -> phdf
            for file_ in files[len(files)//2:]:  # only add last half
                dump = pyharm.load_dump(file_,ghost_zones=False)
                zone = get_zone_num(dump)
                if ur_var_zones[zone] is None: ur_var_zones[zone] = np.power(dump["u^r"]-ur_mean_zones[zone],2.)
                else: ur_var_zones[zone] += np.power(dump["u^r"]-ur_mean_zones[zone],2.)
        for zone in range(n_zones):
            mask = get_mask(zone, n_zones, n_radii)
            dump = dump_zones[zone]
            ur_var_zones[zone] /= num_sum[zone]
            plot_xz(ax[2], dump, ur_var_zones[zone] , native=True, symlog=True, vmin=vmin,vmax=vmax, cbar=0,shading='flat',window=window,average=avg, mask=mask)
    ...
    if show_Be:
        ...
        ax[0].set_title(r'$<u^r>$')
        ax[1].set_title(r'$<(u^r)^2>-<u^r>^2$')
        ax[2].set_title(r'$\sigma^2_{u^r}$')
    fig.tight_layout()
    fig.suptitle(dirtag[7:]+" runs: {}-{}".format(edge_run-iteration*(n_zones-1),edge_run), y=1.02)
    save_path="./plots/"+dirtag
    if not os.path.exists(save_path):
...
def _main():
    ...
    #plot_Mdot_eta(dirtag, **args)
    #compare_FE_slice(dirtag)
    FE_adv_slice(dirtag, show_Be=False)
