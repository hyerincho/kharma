...
then
  ...
  if [[ $ARGS == *"cuda"* ]]; then
    # GPU Compile
    # Default cudatoolkit (13.2) removed the old cudaMemPrefetchAsync(ptr, bytes,
    # device, stream) overload, which the pinned Kokkos version (4.0.00) still
    # uses, breaking the build. Swap to the CUDA 12.9 toolkit, which keeps it.
    module swap cudatoolkit cudatoolkit/26.5_12.9-unsupported
